    ...
    def _handle_Return(
        ...
        if (
            ...
            if isinstance(ret_val, SimRegArg):
                ...
                for ret_val_i in ret_val.locations:
                    reg = self.arch.registers[ret_val_i.reg_name]
                    new_stmt.ret_exprs.append(
                        ailment.Expr.Register(
                            self._next_atom(),
                            None,
                            reg[0],
                            ret_val_i.size * self.arch.byte_width,
                            reg_name=self.arch.translate_register_name(reg[0], ret_val_i.size),
                            )
                    )
            else:
