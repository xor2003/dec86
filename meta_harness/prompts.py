...
def _diagnostic_discipline() -> str:
    return (
        ...
        "- For one-function SORTDEMO garbage or guessed C, capture a bounded stage bundle first: raw asm, COD window, raw angr codegen, post-callsite codegen, post-stack-lowering codegen, then final CLI stdout/stderr.\n"
...
def build_planner_prompt(  # noqa: D103
    ...
    *,
    ...
    prompt = build_master_prompt(cfg) + (
        ...
        "- If the current bad output is one SORTDEMO function with garbage C, repeated raw carriers, or guessed control flow, plan one bounded debug-bundle capture before more semantic edits.\n"
...
def build_worker_prompt(  # noqa: D103
    ...
    *,
    ...
    prompt = build_master_prompt(cfg) + (
        ...
        "- For repeated SORTDEMO one-function reruns, capture or refresh one debug bundle under `.codex_automation/` before another broad read of logs, diffs, or generated C.\n"
        "- Use `scripts/capture_sortdemo_debug_bundle.py` when you need stage visibility on a bad function; work from the bundle paths instead of replaying `git diff` and the same decompile command again.\n"
        ...
        "- Do not rerun `git diff` just to regain context; use it only after code changed and you need the exact touched hunk.\n"
