...
def test_widening_model_accepts_adjacent_register_slices():
    ...
    low_candidate = WideningCandidate(
        _StorageDomainSignature("register", 1, _StorageView(0, 8)), _StorageView(0, 8), low
    )
    high_candidate = WideningCandidate(
        _StorageDomainSignature("register", 1, _StorageView(8, 8)), _StorageView(8, 8), high
    )
...
def test_widening_model_refuses_adjacent_bp_and_ss_stack_slices():
    codegen = _DummyCodegen()
