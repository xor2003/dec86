...
def test_x86_16_recompilable_subset_syntax_checks_pass():
    ...
    assert dos_load_program["used_shape_ok_evidence"] is False
    assert dos_load_program["c_text_source"] == "bounded_live_decompile"
    assert (
        dos_load_program["c_text_source_path"] == "cod/DOSFUNC.COD"
    )
    assert dos_load_program["decompile_path"] == "bounded_live_decompile"
    assert dos_load_program["bounded_live_decompile_outcome"] == "success"
    assert dos_load_program["bounded_live_shape_ok"] is True
    ...
    # Verify syntax and compile checks pass for all cases
    assert all(result["syntax_ok"] for result in results)
    assert all(result["compile_ok"] for result in results)
    # Shape checks pass for all cases except pre-existing bios_clearkeyflags issue
    essential_cases = {"mov_add_ret", "enter_stack", "xor_ret", "push_pop_ret", "strlen_real", "byteops_real", "loadprog_real", "dos_loadOverlay_real", "dos_loadProgram_real"}
