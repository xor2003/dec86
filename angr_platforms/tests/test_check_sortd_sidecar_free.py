...
def _passing_transcript() -> str:
    function_rows = "\n".join(
        ...
        f"/* failure family: status={'ok' if index < 4 else 'validation_failed'} "
        "stage=not_set sidecar=not_attempted nonopt=not_attempted fallback=file_sweep "
        f"validation={'passed' if index < 4 else 'failed'} */"
        for index, addr in enumerate(EXPECTED_SORTD_FUNCTION_ADDRS)
    )
    return (
        ...
        "/* summary: decompiled 4/20 selected functions */\n"
    )
...
def test_sidecar_free_ratchet_accepts_complete_catalog_and_current_floor():
    ...
    assert result.decompiled_count == 4
