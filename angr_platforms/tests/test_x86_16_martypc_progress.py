...
def test_x86_16_martypc_progress_surface_is_deterministic():
    ...
    assert progress[6][3] == "partial"
    assert progress[9][0] == "P2.2"
    assert progress[9][3] == "partial"
...
def test_x86_16_martypc_progress_summary_reports_completion_percentages():
    ...
    assert summary["landed"] == 11
    assert summary["partial"] == 2
    assert summary["open"] == 0
    assert summary["strict_percent"] == 84.62
    assert summary["weighted_percent"] == 92.31
    ...
    assert summary["partial_codes"] == ("P1.2", "P2.2")
