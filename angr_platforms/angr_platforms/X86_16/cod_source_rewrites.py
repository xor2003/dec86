...
def rewrite_known_cod_object_condition_blocks_from_source(c_text: str, metadata: CODProcMetadata | None) -> str:
    ...
    while idx < len(source_lines):
        block, end = _extract_source_if_block_8616(source_lines, idx)
        if not block:
            idx += 1
            continue
        if block[0] in current_lines:
            idx = end + 1
            continue
        anchor = _next_block_anchor_8616(source_lines, end)
        anchor_index = _anchor_index_for_block_8616(current_lines, anchor)
        if anchor_index is None:
            idx = end + 1
            continue
        _insert_or_replace_block_window_8616(current_lines, block, anchor_index)
        c_text = "\n".join(current_lines)
        changed = True
        idx = end + 1
...
def _extract_source_if_block_8616(source_lines: list[str], idx: int) -> tuple[list[str], int]:
    ...
    return source_lines[idx : end + 1], end


def _next_block_anchor_8616(source_lines: list[str], end: int) -> str | None:
...
def _anchor_index_for_block_8616(current_lines: list[str], anchor: str | None) -> int | None:
    if anchor is not None:
        return next((i for i, candidate in enumerate(current_lines) if candidate.strip() == anchor), None)
    return next((i for i in range(len(current_lines) - 1, -1, -1) if current_lines[i].strip() == "}"), None)


def _insert_or_replace_block_window_8616(current_lines: list[str], block: list[str], anchor_index: int) -> None:
...
def rewrite_missing_source_call_lines(c_text: str, metadata: CODProcMetadata | None) -> str:
    def _impl():
        ...
        for source_line in source_lines:
            ...
            if not _should_insert_missing_source_call_line_8616(
                source_line=source_line,
                source_call_names=source_call_names,
                target_call_names=target_call_names,
                current_line_set=current_line_set,
                current_call_names=current_call_names,
            ):
                continue
            insert_at = _call_line_insert_index_8616(current_lines, line_index)
            ...
            _maybe_insert_declaration_for_assigned_call_line_8616(
                source_line=source_line,
...
def _should_insert_missing_source_call_line_8616(
    *,
    source_line: str,
    source_call_names: list[str],
    target_call_names: set[str],
    current_line_set: set[str],
    current_call_names: set[str],
) -> bool:
...
def _call_line_insert_index_8616(current_lines: list[str], line_index: int) -> int:
    for idx in range(line_index, len(current_lines)):
...
def _maybe_insert_declaration_for_assigned_call_line_8616(
    *,
    source_line: str,
    ...
    if declaration_line is None or declaration_line in current_line_set:
        return
    decl_insert_at = _declaration_insert_index_8616(current_lines)
    current_lines.insert(decl_insert_at, declaration_line)
    current_line_set.add(declaration_line)


def _declaration_insert_index_8616(current_lines: list[str]) -> int:
...
def rewrite_collapsed_source_bodies(c_text: str, metadata: CODProcMetadata | None) -> str:
    def _impl():
        ...
        if not lines:
            return c_text
        body_bounds = _body_bounds_8616(lines)
        ...
        source_decl_lines, source_stmt_lines = _source_decl_and_stmt_lines_8616(
            function_body_lines=function_body_lines,
...
def _body_bounds_8616(lines: list[str]) -> tuple[int, int] | None:
...
def _macro_aliases_8616(source_lines: list[str]) -> dict[str, str]:
    aliases: dict[str, str] = {}
    for raw_line in source_lines:
        stripped = raw_line.strip()
        define_match = re.match(r"^#define\s+([A-Za-z_]\w*)\s+(.+)$", stripped)
        if define_match is not None:
            aliases[define_match.group(1)] = define_match.group(2).strip()
    return aliases
...
def _source_decl_and_stmt_lines_8616(
    *, function_body_lines: list[str], macro_aliases: dict[str, str], metadata
    ...
    def _impl():
        ...
        def normalize_decl_type(type_text: str) -> str:
        ...
        for raw_line in function_body_lines:
            stripped = raw_line.strip().lstrip("{").strip().rstrip("}").strip()
            ...
            if decl_match is not None:
                decl_type = normalize_decl_type(decl_match.group("type"))
                for name in (part.strip() for part in decl_match.group("names").split(",")):
                    if not name:
                        continue
                    decl_line = f"{decl_type} {name};{bp_comment(name)}"
            ...
            if call_names:
                ...
                source_stmt_lines.append(stripped)
        return source_decl_lines, source_stmt_lines
