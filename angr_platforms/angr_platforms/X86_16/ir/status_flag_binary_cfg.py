"""Recover bounded binary evidence for direct-callee status-flag reads.
...
callee may read before overwriting. This module owns no instruction semantics
...
class BinaryStatusFlagReadSummary8616:
    """Incoming flag reads plus closed accounting for one bounded traversal."""
...
def summarize_binary_status_flag_entry_reads_8616(
    ...
    *,
    ...
    Callee overwrites are deliberately not claimed by this reads-only summary.
    """
    bounds = _loaded_image_bounds_8616(project)
    if bounds is None or not (bounds[0] <= entry_address < bounds[1]):
