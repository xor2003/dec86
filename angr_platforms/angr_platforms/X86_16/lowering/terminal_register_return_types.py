...
def materialize_terminal_register_return_type_8616(
    ...
    try:
        ...
        result = TerminalRegisterReturnTypeResult8616(
            False, TerminalRegisterReturnTypeStats8616(failure_count=1)
        )
    ...
    if os.environ.get("INERTIA_DEBUG_TERMINAL_RETURN_TYPES") == "1":
        _LOGGER.warning(
            "terminal AX return type candidate: addr=%#x function=%s prototype=%s return=%s explicit=%s",
            cfunc.addr,
    ...
    is_word = (
        TerminalAxReturnLane8616.WORD in states
        and TerminalAxReturnLane8616.LOW not in states
    )
    if not is_word:
        result = TerminalRegisterReturnTypeResult8616(
            False,
