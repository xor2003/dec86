"""Assemble generated function payloads through a structured C syntax tree.

Layer: Tooling/gates.
Responsibility: canonicalize top-level declarations, join ABI-compatible
external prototypes, and use generated definitions as internal contracts. The
assembler never changes a function-body AST and refuses incompatible external,
aggregate, or global declarations.
"""
...
def _integer_return_contract(node: c_ast.Node) -> tuple[int, bool] | None:
    ...
    if token_set in {frozenset({"int8_t"}), frozenset({"char"}), frozenset({"signed", "char"})}:
        return 8, False
    ...
    if token_set in {
        ...
        return 16, False
    if token_set in {frozenset({"uint16_t"}), frozenset({"unsigned", "short"}), frozenset({"unsigned", "int"})}:
        return 16, True
    if token_set in {frozenset({"int32_t"}), frozenset({"long"}), frozenset({"signed", "long"})}:
        return 32, False
...
def _canonical_external_function_decl(nodes: Sequence[c_ast.Node]) -> c_ast.Decl | None:
    ...
    exact_return_contracts = tuple(_type_contract(declaration.type.type) for declaration, _contracts in typed)
    if len(set(exact_return_contracts)) == 1:
        return typed[0][0]
...
def _declaration_identity(node: c_ast.Node, generator: c_generator.CGenerator) -> tuple[str, str] | None:
...
def _declaration_kind(node: c_ast.Node) -> DeclarationContractKind:
    ...
    if isinstance(node, c_ast.Typedef) or (
        isinstance(node, c_ast.Decl) and isinstance(node.type, (c_ast.Struct, c_ast.Union, c_ast.Enum))
    ):
